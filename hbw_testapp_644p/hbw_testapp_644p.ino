/*
 * HBW-Test-App (ATmega644P / 644PA)  —  App-Seite fuer den HBW-Booter, 644P-Portierung
 * -----------------------------------------------------------------------------------
 * Wie hbw_testapp (328P), aber fuer den ATmega644P/644PA (64 KB Flash). Normale
 * App-Section (0x0000), spricht das HMW-Wire-Protokoll (Announce, h/v/n, z/Z). Auf 'u'
 * (START_BOOTER): ACK + ECHTER Watchdog-Reset  ->  der Booter (@0xF000) uebernimmt.
 *
 * Der 644P ist der einfachste Fall: 64 KB Flash passen KOMPLETT in die 16-bit-w-Adresse
 * (kein Bank-Byte), und mangels >64 KB gibt es gar kein RAMPZ. Der Code ist identisch zur
 * 1284P-Test-App -- nur die Boot-Section liegt bei 0xF000 statt 0x1F000.
 *
 * BUILD (arduino-cli, MightyCore) -- ohne MightyCore-Bootloader, wir setzen eigene Fuses:
 *   arduino-cli compile --fqbn MightyCore:avr:644:variant=modelP,clock=16MHz_external,pinout=standard,bootloader=no_bootloader \
 *       hbw_testapp_644p
 * Danach mit dem Booter zu einer ISP-Datei mergen:
 *   python merge_hex.py hbw_testapp_644p.hex hbw_booter_atmega644p.hex hbw_combined_644p.hex
 *
 * DE-Pin: der Booter treibt PD2 direkt -- diese App tut das ebenso (Portregister statt
 * Arduino-Pin-Nummer), damit App und Booter am RS485 pinout-unabhaengig denselben
 * Transceiver-DE ansteuern. Beim reinen USB-Test ist DE ohne Funktion (harmlos).
 * Serial = UART0 (PD0/PD1), genau wie im Booter.
 */
#include <avr/wdt.h>
#include <avr/io.h>
...
#define USE_DE       1            // DE ueber PD2 (wie im Booter); beim USB-Test harmlos
#define OWN_ADDRESS  0x42FFFFFFUL // = Booter-Fallback bei leerem EEPROM -> beide gleich
#define DEVICE_TYPE  0x10         // fuer den reinen Mechanismus-Test egal (Python prueft ihn nicht)
#define HW_VERSION   0x00
#define FW_VERSION   0x0003       // App-Version (>Booter 0x0001, zur Unterscheidung im Announce)
// =================================================================
...
// DE ueber PD2 direkt -- konsistent mit dem Booter (DDRD/PORTD Bit 2), pinout-unabhaengig.
static void deInit(){
#if USE_DE
  DDRD |= (1<<PD2); PORTD &= ~(1<<PD2);   // Ausgang, Zustand Empfang
#endif
}
static void deTx(uint8_t on){
#if USE_DE
  if(on) PORTD |= (1<<PD2); else PORTD &= ~(1<<PD2);
#else
  (void)on;
#endif
}
...
static void handleFrame(uint32_t target,uint8_t control,uint32_t sender,uint8_t* data,uint8_t dlen){
  ...
  switch(cmd){
    case CMD_START_BOOTER:            // 'u' -> ACK, dann ECHTER Watchdog-Reset
      ...
      wdt_enable(WDTO_15MS);          // Watchdog-Reset in 15 ms -> Booter uebernimmt (sieht WDRF)
