/*
 * HBW-Test-App (ATmega1284P)  —  App-Seite fuer den HBW-Booter, 1284P-Portierung
 * -----------------------------------------------------------------------------
 * Wie hbw_testapp (328P), aber fuer den ATmega1284P. Normale App-Section (0x0000),
 * spricht das HMW-Wire-Protokoll (Announce, h/v/n, z/Z). Auf 'u' (START_BOOTER):
 * ACK + ECHTER Watchdog-Reset  ->  der Booter (@0x1F000) uebernimmt.
 *
 * Zweck: Erst-Test des 1284P-Booters ueber USB-Serial (ohne RS485/CCU) -- exakt der
 * Weg, mit dem der 328P-Booter verifiziert wurde. Python schickt z z u (booter_test.py)
 * bzw. flasht (flash_tool.py); die App muss nur announcen und auf 'u' den WDT-Reset ausloesen.
 *
 * BUILD (arduino-cli, MightyCore) -- ohne MightyCore-Bootloader, wir setzen eigene Fuses:
 *   arduino-cli compile --fqbn MightyCore:avr:1284:pinout=standard,variant=modelP,clock=16MHz_external \
 *       hbw_testapp_1284p
 * Danach mit dem Booter zu einer ISP-Datei mergen:
 *   python merge_hex.py hbw_testapp_1284p.hex hbw_booter_atmega1284p.hex hbw_combined_1284p.hex
 *
